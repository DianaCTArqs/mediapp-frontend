...
import { VitalSign } from 'src/app/model/VitaSign';
...
export class VitalSignEditComponent implements OnInit {
  ...
  operate() {
    ...
    vitalSign.date = moment(this.form.value['date']).format('YYYY-MM-DDT HH:mm:ss');
    ...
    if (this.isEdit) {
      ...
      this.vitalSignService.update(vitalSign).subscribe(() => {
        this.vitalSignService.findAll().subscribe(data => {
          ...
          this.vitalSignService.setMessageChange('successful update')
      ...
      this.vitalSignService.save(vitalSign).pipe(switchMap(()=>{        
        ...
        this.vitalSignService.setVitalSignChange(data);
        this.vitalSignService.setMessageChange("successful creation")
